...
// User management is handled through adminService
...
const AdminUsers = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        setIsLoading(true);
        const response = await adminService.getRecentUsers();
  ...
  const handleAction = async (action: string, user: UserData) => {
    ...
    if (action === "view") {
      ...
      try {
        await adminService.manageUser(user.id, {}, "DELETE");
  ...
  const handleSubmit = async () => {
    try {
      if (selectedUser) {
        ...
        const response = await adminService.manageUser(
          selectedUser.id,
          updateData,
          "PUT"
        );
        ...
        setUsers(
          users.map((user) =>
            user.id === selectedUser.id ? { ...response, status } : user
          )
        );
        ...
        // Create new user - using auth register endpoint since it's not in adminService
        // This would ideally be added to adminService in the future
